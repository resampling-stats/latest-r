...
As you saw in Section , we do *indexing* when we put square brackets
following a value that is a container, such as a vector. Inside the
square brackets we put another value to specify which elements we want
to fetch from the container.
...
See Note  for more on operators. The colon operator (the `:`) serves as
a signal to R to create a new value by *operating* on the values to the
left and right of the colon.
