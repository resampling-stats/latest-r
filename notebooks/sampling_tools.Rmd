...
elements we are sampling from. For example, in Chapter , we were
...
Here is the process of simulating a single juror, adapted from Section :
...
__quarto_custom_context="Block" __quarto_custom_id="544">
...
(12) with the second argument to `sample`. As you saw in Section , we
can also give names to the function arguments, in this case, to make it
...
Finally, we can `sum` to find the number of black jurors (Section ):
...
from Chapter  to ask the question — is 8% surprising as a random sample
...
<div __quarto_custom="true" __quarto_custom_type="Callout"
__quarto_custom_context="Block" __quarto_custom_id="545">
