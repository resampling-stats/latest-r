...
`rep` is a useful function in R that makes a new output vector by
repeating elements from the input vectors.

More specifically, `rep` accepts two arguments (inputs), and sends back
a vector as output. The first input is a vector for which you want to
repeat elements. The second input tells `rep` how many repeats to do,
for each element.

This is most easy to see by examples.

Here we repeat all three elements in the first vector by a count
(number) given in the second argument.
...
The second argument (number of repeats) can also be a vector with one
element for each element of the first vector. In that case, it gives the
number of repeats for each element. We will use string elements for this
example.
...
Here we use `rep` to make up our simulated jury pool.
...
<a class="quarto-xref" href="https://resampling-stats.github.io/latest-r/resampling_with_code.html#sec-named-arguments"><span>Section 5.8</span></a>, we can
...
(<a class="quarto-xref" href="https://resampling-stats.github.io/latest-r/resampling_with_code.html#sec-count-with-sum"><span>Section 5.15</span></a>):
...
from <a class="quarto-xref" href="https://resampling-stats.github.io/latest-r/resampling_with_code2.html"><span>Chapter 6</span></a> to ask the
